    ...
    if(_isLoggedIn) {
      ...
      for(ZoneData zData in Get.find<LocationController>().getUserAddress()!.zoneData!){
        if(zData.id ==  Get.find<LocationController>().getUserAddress()!.zoneId){
          ...
          _isPluginPaymentActive = Get.find<SplashController>().configModel!.digitalPaymentInfo!.pluginPaymentGateways!;
              ...
              return _isLoggedIn ? Column(children: [
                ...
                Expanded(child: SingleChildScrollView(
                  ...
                  child: FooterView(child: SizedBox(width: Dimensions.webMaxWidth, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    (!_isPluginPaymentActive! && _isDigitalPaymentActive! && parcelController.payerIndex == 0) ? PaymentButton(
                    ...
                    (_isPluginPaymentActive! && _isDigitalPaymentActive! && parcelController.payerIndex == 0) ? Column(children: [
                      Row(children: [
      ...
      if(Get.find<ParcelController>().paymentIndex == 2) {
        if(GetPlatform.isWeb) {
          ...
          String selectedUrl = '${AppConstants.baseUrl}/payment-mobile?order_id=$orderID&&customer_id=${Get.find<UserController>()
              .userInfoModel!.id}&payment_method=${Get.find<ParcelController>().digitalPaymentName}&&callback=$protocol//$hostname${RouteHelper.orderSuccess}?id=$orderID&status=';
          html.window.open(selectedUrl,"_self");
        } else{
          Get.offNamed(RouteHelper.getPaymentRoute(orderID, Get.find<UserController>().userInfoModel!.id, 'parcel', orderAmount, _isCashOnDeliveryActive, Get.find<ParcelController>().digitalPaymentName));
        }
      }else {
      ...
      onPressed: parcelController.acceptTerms ? () {
        if(parcelController.distance == -1) {
          ...
          Get.find<OrderController>().placeOrder(PlaceOrderBody(
            ...
            paymentMethod: parcelController.paymentIndex == 0 ? 'cash_on_delivery' : parcelController.paymentIndex == 1 ? 'wallet' : 'digital_payment',

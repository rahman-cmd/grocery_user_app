...
class WebItemsView extends StatefulWidget {
  ...
  const WebItemsView({Key? key, required this.stores, required this.items, required this.isStore, this.isScrollable = false,
    ...
    this.isCampaign = false, this.inStorePage = false, this.isFeatured = false, this.showTheme1Store = false, this.fromStore = false}) : super(key: key);
    ...
    return Column(children: [
      ...
      !isNull ? length > 0 ? GridView.builder(
        ...
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
          ...
          childAspectRatio: ResponsiveHelper.isDesktop(context) ? widget.isStore ? 1.1 : 0.9 : widget.showTheme1Store ? 2 : 1,
        ...
        itemBuilder: (context, index) {
          return widget.showTheme1Store ? StoreWidget(store: widget.stores![index], index: index, inStore: widget.inStorePage, fromWebSearch: true)
              : WebItemWidget(
